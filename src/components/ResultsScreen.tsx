    ...
    <div className="flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-emerald-900/30 rounded-xl p-4 text-center">
          <p className="text-sm text-emerald-400 mb-1">Me</p>
          <p data-testid="total-me" className="text-3xl font-bold text-emerald-300">
            {formatPrice(meOwes)} €
          </p>
        </div>
        <div className="bg-blue-900/30 rounded-xl p-4 text-center">
          <p className="text-sm text-blue-400 mb-1">You</p>
          <p data-testid="total-you" className="text-3xl font-bold text-blue-300">
            {formatPrice(youOwes)} €
          </p>
        </div>
      </div>
      ...
      {result.commonTotal > 0 && (
        <div className="bg-purple-900/30 rounded-lg p-3 mb-4 text-center">
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 text-slate-400 hover:text-slate-200 mb-4"
      >
            ...
            <div>
              ...
              <div className="space-y-1">
                {result.ignoredItems.map(({ item }) => (
                  <div key={item.id} className="flex justify-between text-sm text-slate-500 line-through">
                    <span>{item.name}</span>
                    <span>{formatPrice(item.price)} €</span>
                  </div>
                ))}
      ...
      <div className="space-y-3 mt-auto">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-medium py-3 px-4 rounded-lg transition-colors"
        >
          {copied ? (
            <>
              <span>✓</span>
        ...
        <button
          ...
          className="w-full text-slate-400 hover:text-slate-200 font-medium py-3 px-4 transition-colors"
        >
